...
public class ComplaintsService {
	...
	public OperationResponse setCodeAction(OperationRequest actionRequest, ExtensionHelper extensionHelper) {
		 ...
		 * ) .header("cache-control", "no-cache").field("texts",
		 * text).asJson().getBody().getObject();
		 * 
		 * code = (String) json.query("/predictions/0/results/0/label"); // Double score
		 * = (Double)json.query("/predictions/0/results/0/score");
		 * 
		 * 
		 * } else {
		 * 
		 ...
		 * .body("{ \"payload\": { \"textSnippet\": { \"content\": \"" + text +
		 * "\", \"mime_type\": \"text/plain\" } } }") .asJson().getBody().getObject();
		 */
		String auth = readCredentialsFromVCAP(); // "Bearer
													// ya29.c.Kl6wBx-vkdVRLUhth34YgZ2FGZ407J2OphqN0oEM4zxUAmTVwjWGw5uwdh96rMbQRro9mSO0c44FnhCl7SFONERHsJxX33fvkqCjdNnF7jV0B-tU1UKeTCRzdRJI6d_R";

		LOG.error("Token:" + auth);
		LOG.error("Text:" + text);
		LOG.error("Entity:" + entityId);

		String json = Unirest.post(
				"https://example.com/google_api/v1beta1/projects/663868982523/locations/us-central1/models/TCN3178648533481816064:predict")
				.header("content-type", "application/json").header("authorization", auth)
				.header("cache-control", "no-cache").body("{ \"payload\": { \"textSnippet\": { \"content\": \"" + text
		...
		if (code == null || code.equals("")) {
			if (text.toLowerCase().contains("preis")) {
				code = "10-SC-SD-ERFA";
			} else if (text.toLowerCase().contains("krat")) {
				code = "40-HC-TE-KRAT";
			}
		}
